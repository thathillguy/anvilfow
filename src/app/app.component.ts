...
import {loadAllUnits, loadAllAbilities, loadCoreRules} from "./store/app.actions";
import { selectActiveUnitID, selectAllAbilities, selectAllUnits, selectArmyUnits, selectCoreRules } from './store/app.selector';
...
export class AppComponent implements OnInit {
  ...
  coreRuleData$: Observable<Ability[]>;
  ...
  constructor(private store: Store<AppState>) {
    ...
    //Select and subscribe for Core Rules
    this.coreRuleData$ = store.pipe(
      select(selectCoreRules)
    )
    this.coreRuleData$.subscribe(
      data => {console.log("Core Rule Subscribe", data)}
    )
  }

  ngOnInit() {
    console.log("onInit AppComponent");
    this.store.dispatch(loadCoreRules())
